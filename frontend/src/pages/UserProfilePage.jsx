...
import {
  ...
  FaGlobe, FaPlus, FaTh, FaList, FaStar,
  FaFire, FaBookOpen, FaCamera, FaSave, FaTimes
} from 'react-icons/fa';
import { useAuth } from '../contexts/AuthContext';
import { getRoleDisplayName, getRoleBadgeColor, canCreateRecipes } from '../utils/roleUtils';
...
const UserProfilePage = () => {
  ...
  const [recipeCounts, setRecipeCounts] = useState({ total: 0, published: 0, draft: 0 });
  ...
  useEffect(() => {
    console.log('Frontend: UserProfilePage loaded, userId:', userId, 'isOwnProfile:', isOwnProfile);
    ...
    if (activeTab === 'recipes' && profileUser) {
      ...
      const loadUserRecipes = async () => {
        ...
        try {
          ...
          console.log('Frontend: recipes length:', response.data.recipes?.length || 0);
          ...
          const transformedRecipes = (response.data.recipes || []).map(recipe => ({
            ...recipe,
            author: recipe.user?.username || recipe.user?.full_name || profileUser?.username || 'Unknown Chef',
          ...
          console.log('Frontend: transformed recipes length:', transformedRecipes.length);
            ...
            const loadRecipeCounts = async () => {
              try {
                ...
                const [allResponse, publishedResponse, draftResponse] = await Promise.all([
                  apiClient.get('/auth/users/me/recipes?status=all&per_page=1000'),
                  apiClient.get('/auth/users/me/recipes?status=published&per_page=1000'),
                  apiClient.get('/auth/users/me/recipes?status=draft&per_page=1000')
                ]);
  ...
  }, [activeTab, currentPage, profileUser, recipeStatus, isOwnProfile, refreshTrigger]);
        ...
        <div className="container px-4 py-12 mx-auto">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="p-8 bg-white shadow-xl rounded-2xl">
                  ...
                  <div className="flex flex-col mb-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-2 mb-4 lg:justify-start">
                        <div className={`px-3 py-1 rounded-full text-sm font-medium ${getRoleBadgeColor(profileUser.role)}`}>
              ...
              {/* Stats Section */}
              <div className="grid grid-cols-1 gap-4 pt-8 mt-8 border-t lg:grid-cols-2">
                {/* Only show recipe stats for users who can create recipes */}
      ...
      <div className="container px-4 py-8 mx-auto">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="p-1 mb-8 bg-white tabs tabs-boxed">
            {/* Only show Recipes tab for users who can create recipes OR have existing recipes */}
            ...
            <div>
              <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  {/* Recipe Status Filter - Only for own profile */}
                  {isOwnProfile && (
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-600">Status:</span>
                      <div className="btn-group">
                        <button
                          className={`btn btn-xs ${recipeStatus === 'all' ? 'btn-active btn-primary' : 'btn-outline'}`}
                          onClick={() => handleRecipeStatusChange('all')}
                        >
                          All ({recipeCounts.total})
                        </button>
                        <button
                          className={`btn btn-xs ${recipeStatus === 'published' ? 'btn-active btn-success' : 'btn-outline'}`}
                          onClick={() => handleRecipeStatusChange('published')}
                        >
                          ✅ Published ({recipeCounts.published})
                        </button>
                        <button
                          className={`btn btn-xs ${recipeStatus === 'draft' ? 'btn-active btn-warning' : 'btn-outline'}`}
              ...
              {recipesLoading ? (
                ...
                <>
                  {console.log('Frontend: Rendering recipes, userRecipes state:', userRecipes)}
                  {console.log('Frontend: userRecipes.length:', userRecipes.length)}
                  ...
                  <div className="card-body">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="stat">
                        ...
                        <div className="text-base stat-value">
                          {userRecipes.length > 0 ? (
                            (userRecipes.reduce((total, recipe) => total + (recipe.rating || 0), 0) / userRecipes.length).toFixed(1)
                          ) : '0.0'} ⭐
                        </div>
                      </div>
                    </div>
