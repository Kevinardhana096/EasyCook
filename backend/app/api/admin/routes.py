...
def create_admin_user():
    """Create admin user for testing - ONLY for development"""
    try:
        ...
        admin = User(
            ...
            full_name='CookEasy Administrator',
...
@admin_bp.route('/test-roles', methods=['POST'])
def create_test_users():
    """Create test users with different roles - ONLY for development"""
    try:
        test_users = [
            {
                'username': 'chef_mario',
                'email': 'mario@cookeasy.com',
        ...
        for user_data in test_users:
            ...
            user = User(
                ...
                bio=f'Test {user_data["role"]} account',
                role=user_data['role'],
                is_active=True,
                is_verified=True
            )
        ...
        db.session.rollback()
        return jsonify({'message': 'Failed to create test users', 'error': str(e)}), 500
